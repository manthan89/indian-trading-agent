    ...
    <div className="p-6 space-y-6">
      ...
      {closedTrades.length > 0 && (
        <div className="grid grid-cols-4 gap-4">
          <Card><CardContent className="p-4 text-center"><p className="text-xs text-muted-foreground">Win Rate</p><p className="text-xl font-bold">{winRate}%</p><p className="text-xs text-muted-foreground">{wins}W / {losses}L</p></CardContent></Card>
          <Card><CardContent className="p-4 text-center"><p className="text-xs text-muted-foreground">Avg Return</p><p className={`text-xl font-bold ${totalPnl / closedTrades.length >= 0 ? "text-green-600" : "text-red-600"}`}>{(totalPnl / closedTrades.length).toFixed(2)}%</p></CardContent></Card>
          <Card><CardContent className="p-4 text-center"><p className="text-xs text-muted-foreground">Total P&L %</p><p className={`text-xl font-bold ${totalPnl >= 0 ? "text-green-600" : "text-red-600"}`}>{totalPnl >= 0 ? "+" : ""}{totalPnl.toFixed(2)}%</p></CardContent></Card>
          <Card><CardContent className="p-4 text-center"><p className="text-xs text-muted-foreground">Closed Trades</p><p className="text-xl font-bold">{closedTrades.length}</p>{openTrades.length > 0 && <p className="text-xs text-muted-foreground">{openTrades.length} open</p>}</CardContent></Card>
        </div>
      )}
      ...
      <Tabs defaultValue="all">
        <TabsList>
          ...
          <TabsTrigger value="open"><Clock className="h-3 w-3 mr-1" /> Open ({openTrades.length})</TabsTrigger>
          <TabsTrigger value="closed"><CheckCircle2 className="h-3 w-3 mr-1" /> Closed ({closedTrades.length})</TabsTrigger>
            ...
            <Card>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Date</TableHead><TableHead>Ticker</TableHead><TableHead>Trade Date</TableHead>
                      <TableHead>Signal</TableHead><TableHead className="text-right">Entry</TableHead><TableHead className="text-right">Exit</TableHead>
                      <TableHead className="text-right">P&L</TableHead><TableHead>Result</TableHead><TableHead className="text-right">Duration</TableHead><TableHead></TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {loading ? (
                      ...
                      tab.data.map((a: any) => (
                        <TableRow key={a.task_id}>
                          ...
                          <TableCell><Badge variant="outline" className={signalColors[a.signal] || ""}>{a.signal}</Badge></TableCell>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <Link href={`/app/analysis/${a.task_id}`} className="text-xs text-primary hover:underline">View</Link>
                              <Button size="sm" variant="ghost" className="h-6 px-2 text-xs" onClick={() => setDialog({ taskId: a.task_id, ticker: a.ticker, signal: a.signal })}>
      ...
      {dialog && <PnLDialog open={!!dialog} onClose={() => setDialog(null)} taskId={dialog.taskId} ticker={dialog.ticker} signal={dialog.signal} onSaved={load} />}
    </div>
  );
}